...
% harmonic oscillator (ho), using a Metropolis algorithm [1]. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function [] = vmc_one_dim_ho_energy
%
clc; clear; format long
%
dbeta = 0.1;
beta = 0.1:dbeta:1.300;
%
fileID_save_data_1 = fopen('vmc_one_dim_ho_energy.txt','w');
%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
read_data = fopen('vmc_one_dim_ho_energy.txt', 'r');               % 
read_data = textscan(read_data, '%f %f %f %f %f');
beta_vals = read_data{2};
En0_vals = read_data{3};
%

En0_exact_value = 0.5; % an exact value of ground state of one-dimensional harmonic oscillator 
...
for moves = 1:2*n_moves
    ...
    if (moves < n_moves)
        %
        sm_en0 = sm_en0 + (beta - 2.*beta.^2.*x.^2) + 0.5.*x.^2;
